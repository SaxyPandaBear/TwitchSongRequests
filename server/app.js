...
function validateSpotifyAuthRequestBody(body) {
    if (!("authorization_code" in body)) {
        console.warn(`Request ${JSON.stringify(body)} does not contain an authorization code.`);
        return false;
    } else if (!("id" in body)) {
        console.warn(`Request ${JSON.stringify(body)} does not contain a Twitch channel ID.`);
        return false;
    }
