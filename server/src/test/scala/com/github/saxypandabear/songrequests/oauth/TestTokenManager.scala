...
class TestTokenManager(clientId: String, clientSecret: String, refreshToken: String, uri: String)
    ...
    override def getAccessToken: String = {
        TestTokenManager.clientIdsToTokens.getOrElseUpdate(clientId, UUID.randomUUID().toString)
    }
    ...
    override def refresh(): OauthResponse = {
        if (TestTokenManager.acceptedCredentials.exists(tuple =>
            tuple._1 == clientId && tuple._2 == clientSecret && tuple._3 == refreshToken)) {
            // first update the token in our internal map so we can assert it properly in tests
